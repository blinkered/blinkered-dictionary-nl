/**
 * The collections that attest Dutch, and where each comes from.
 *
 * Dutch is well supplied: 213,509 Internet Archive texts and 1,114 Gutenberg books
 * against a 322,146-word candidate list.
 *
...
// wearing a Leipzig label, so including one would corroborate `wiki:nl` while looking
...
const ALL = [
  {
    id: 'wiki:nl',
    what: 'Dutch Wikipedia — modern encyclopedic prose',
    needs: `${CACHE}nlwiki.xml.bz2`,
    documents: () => wikiDocuments(`${CACHE}nlwiki.xml.bz2`),
  },
  {
    id: 'wikisource:nl',
    what: 'Dutch Wikisource — same Wikimedia family, so it corroborates rather than counts',
    needs: `${CACHE}nlwikisource.xml.bz2`,
    documents: () => wikiDocuments(`${CACHE}nlwikisource.xml.bz2`),
  },
  ...LEIPZIG.map((pkg) => ({
    id: `lz:${pkg}`,
  ...
  {
    id: 'tat',
    from: 'https://downloads.tatoeba.org/exports/per_language/nld/nld_sentences.tsv.bz2',
    what: 'Tatoeba Dutch — contemporary and conversational',
    needs: `${CACHE}nld_sentences.tsv`,
    documents: () => tatoebaDocuments(`${CACHE}nld_sentences.tsv`),
  },
  {
    id: 'fw2',
    from: 'https://huggingface.co/datasets/HuggingFaceFW/fineweb-2/resolve/main/data/nld_Latn/train/000_00000.parquet',
    what: 'FineWeb-2 Dutch — a web crawl nobody here made',
    needs: `${CACHE}fineweb2-nld.parquet`,
    documents: () => fineweb2Documents(`${CACHE}fineweb2-nld.parquet`),
  },
  {
    id: 'gut',
    from: 'https://www.gutenberg.org/cache/epub/feeds/pg_catalog.csv',
    what: 'Project Gutenberg Dutch, 1,114 texts',
    needs: `${CACHE}gutenberg-nl`,
    documents: () => {
      const dir = `${CACHE}gutenberg-nl`
  ...
  {
    id: 'ebible:nld',
    from: 'https://ebible.org/Scriptures/nld_vpl.zip',
    what: 'Statenvertaling — a family nothing else here belongs to',
    needs: `${CACHE}ebible-nld/nld_vpl.txt`,
    documents: () => verseDocuments(`${CACHE}ebible-nld/nld_vpl.txt`),
  },
  {
    id: 'ia',
    ...
    legible: 0.35,
    what: 'Internet Archive Dutch books — literature, and the register a newspaper never reaches',
    needs: `${CACHE}archive-nl`,
    from: 'https://archive.org/search?query=mediatype%3Atexts+AND+language%3A%22Dutch%22',
    documents: () => {
      const dir = `${CACHE}archive-nl`
...
/**
 * Dutch publishers, for the harvest.
 *
 * Chosen because they publish in Dutch rather than because they are large. A harvester
...
export const DOMAINS = [
  'nos.nl', 'nrc.nl', 'volkskrant.nl', 'trouw.nl', 'parool.nl', 'ad.nl',
  'telegraaf.nl', 'fd.nl', 'nu.nl', 'rtlnieuws.nl',
  'standaard.be', 'demorgen.be', 'tijd.be', 'knack.be', 'vrt.be',
  'dbnl.org', 'tzum.info', 'literatuurplein.nl', 'deburen.eu',
]
